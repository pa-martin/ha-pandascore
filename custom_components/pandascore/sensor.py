...
def _create_team_sensors(coordinator: PandascoreDataUpdateCoordinator, team: dict[str, Any]) -> list[SensorEntity]:
    ...
    return [
        PandascoreNameSensor(coordinator, team, team_id,
                             team_slug, device_info),
        PandascoreGameSensor(coordinator, team, team_id,
                             team_slug, device_info),
        PandascoreNextMatchSensor(
            coordinator, team, team_id, team_slug, device_info),
        PandascoreLastMatchSensor(
            coordinator, team, team_id, team_slug, device_info),
        PandascoreMatchesWonSensor(
            coordinator, team, team_id, team_slug, device_info),
        PandascoreMatchesLossSensor(
            coordinator, team, team_id, team_slug, device_info),
        PandascoreWinRateSensor(
            coordinator, team, team_id, team_slug, device_info),
...
class PandascoreTeamSensorBase(CoordinatorEntity[PandascoreDataUpdateCoordinator], SensorEntity):
...
class PandascoreMatchesLossSensor(PandascoreTeamSensorBase):
    ...
    def native_value(self) -> int:
        ...
        if not data:
            return 0
        loss = sum(1 for m in data.matches if m.winner_id !=
                   self._team_id and m.status == "finished")
        return loss
...
class PandascoreMatchesPlayedSensor(PandascoreTeamSensorBase):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        return {"matches": [build_match_entry(self._team_id, m, include_result=True) for m in past]}
...
class PandascoreUpcomingMatchesSensor(PandascoreTeamSensorBase):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if not data:
            return {"matches": []}
        upcoming = [m for m in data.matches if m.status in {
            "not_started", "pending"}]
        upcoming.sort(
