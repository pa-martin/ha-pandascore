...
def build_match_entry(team_id: int, match: Match, include_result: bool = False) -> dict[str, Any]:
    """TODO"""
    ...
    if scheduled and not end_at:
        ...
        end_at = scheduled + \
            timedelta(milliseconds=estimate_duration_ms(
                videogame_id, number_of_games))
...
def extract_score(match: Match) -> str | None:
    ...
    if results:
        try:
            return f"{results[0].score}-{results[1].score}" if len(results) > 1 else None
        except (IndexError, TypeError):
            pass
    return None


async def build_team_tracker(hass: HomeAssistant, match: Match, language: str | None) -> dict[str, Any]:
    """TODO"""
    opponent_1 = match.opponents[0].opponent if match.opponents[0].type == "Team" else match.opponents[0]
    opponent_2 = match.opponents[1].opponent if match.opponents[1].type == "Team" else match.opponents[1]
    op_win_rate = 0 if 'karmine' in opponent_1.slug or 'karmine' in opponent_2.slug else 0.5

    entry_dict = await build_match_attributes(hass, match, language)
    entry_dict.update(build_team_attributes(
        opponent_1, match.results, "team_", op_win_rate))
    entry_dict.update(build_team_attributes(
        opponent_2, match.results, "opponent_", op_win_rate))

    return entry_dict


async def build_match_attributes(hass: HomeAssistant, match: Match, language: str | None) -> dict[str, Any]:
    """TODO"""
    local_streams = [
    ...
    state = "PRE"
    relative = await get_relative_date(hass, (match.begin_at or match.scheduled_at) or now(), language or 'en')
    if match.status == 'running':
        state = "IN"
    if match.status in {"finished", "canceled"}:
        state = "POST"
        relative = await get_relative_date(hass, match.end_at or now(), language or 'en')
    ...
    return {
        ...
        # PRE
        "odds": translate(language, "down_distance_text") % str(get_bo(match.number_of_games or 0)),
        "down_distance_text": get_full_game_name(match),  # IN
        # Row 2
        "location": match.videogame_version.name if match.videogame_version else 'No version',
...
def build_team_attributes(opponent: Opponent, results: list[Result], side: Literal['team_', 'opponent_'],
                          op_win_rate: int) -> dict[str, Any]:
    """TODO"""
    return {
        side + "name": opponent.name or "Unknown",
        side + "logo": opponent.image_url or "",
        side + "logo_dark": opponent.dark_mode_image_url or "",
        side + "score": results[0].score if results[0].team_id == opponent.id else results[1].score,
        side + "timeouts": results[0].score if results[0].team_id == opponent.id else results[1].score or 0,
        ...
        side + "winner": results[0].team_id == opponent.id and results[0].score > results[1].score or results[
            1].team_id == opponent.id and results[1].score > results[0].score,
        side + "record": None,
        side + "rank": None,
    }
...
async def get_relative_date(hass: HomeAssistant, date: datetime, language: str) -> str:
    """TODO"""
    f_kwargs = {"delta": date - now(), "add_direction": True,
